...
env_vars=(BROWSER CLUTTER_BACKEND COLORTERM CUPS_DATADIR \
  ...
  LC_TELEPHONE LC_TIME LESSKEYIN_SYSTEM LESSOPEN LIBEXEC_PATH LOCALE_ARCHIVE \
  LOCALE_ARCHIVE_2_27 LOGNAME MOZ_ENABLE_WAYLAND MOZ_PLUGIN_PATH NIXOS_OZONE_WL \
  NIXPKGS_CONFIG NIX_PATH NIX_PROFILES NIX_USER_PROFILE_DIR NO_AT_BRIDGE OLDPWD \
  PAGER PATH PWD QTWEBKIT_PLUGIN_PATH QT_PLUGIN_PATH QT_QPA_PLATFORM \
  QT_WAYLAND_DISABLE_WINDOWDECORATION QT_WAYLAND_FORCE_DPI SAVEHIST \
  SDL_VIDEODRIVER SHELL SHLVL SSH_ASKPASS SSH_AUTH_SOCK SWAYSOCK TERM TERMINFO \
  TERMINFO_DIRS TZDIR USER VDPAU_DRIVER WAYLAND_DISPLAY WLR_LIBINPUT_NO_DEVICES \
  XCURSOR_PATH XCURSOR_SIZE XCURSOR_THEME XDG_BIN_HOME XDG_CACHE_HOME \
  XDG_CONFIG_DIRS XDG_CONFIG_HOME XDG_CURRENT_DESKTOP XDG_DATA_DIRS \
  XDG_DATA_HOME XDG_DESKTOP_PORTAL_DIR XDG_RUNTIME_DIR XDG_SEAT \
